	...
	movieApp.router = {
		init: function() {
			routie({
				...
				// Genre: (movies/detail/genre)
				"movies/genres/:argument": function(argument) {
					movieApp.sections.toggle("movies", "genres", argument)
				},
				// Detail: (movies/detail/999/toy-story)
				"movies/detail/:argument/*": function(argument) {
					movieApp.sections.toggle("movies", "detail", argument)
				},
			    "*": function() {
			    	movieApp.sections.toggle("about");
			    }
	...
	movieApp.sections = {
		...
		toggle: function(section, subPage, argument) {
			...
			for (i = 0; i < selector.length; i++) {
				...
				// Verwijder bij alle <section> de CSS class "detail"
				selector[i].classList.remove("detail");
			}
			...
			*/
			...
			// Als de doorgestuurde <section> "movies" is dan zijn er drie opties:
				...
				if( !subPage ) {
					...
					// Verwijder bij alle .simplePlot de CSS class "detail"
					var simplePlot = document.querySelectorAll(".simplePlot");
					for (i = 0; i < simplePlot.length; i++) {
						simplePlot[i].classList.add("active");
					}
				}
				...
				// NUMMER 2: Als er een subPage doorgestuurd is en deze subPagina 'detail' is
					...
					var simplePlot = document.querySelectorAll(".simplePlot");
					for (i = 0; i < simplePlot.length; i++) {
						simplePlot[i].classList.remove("active");
					}
				...
				//  NUMMER 3: Als er een subPage doorgestuurd is en deze subPagina 'genres' is
					...
					pageTitle.classList.add("active");
					pageTitle.innerHTML = "Genre: " + argument;
