...
// Content js bestand wordt als eerste ingeladen
...
"<p>That tall drink of water with the silver spoon up his ass. well, what is it today? more spelunking? i now issue a new commandment: thou shalt do the dance. let me tell you something my friend. hope is a dangerous thing. hope can drive a man insane. i did the same thing to gandhi, he didn't eat for three weeks. the man likes to play chess; let's get him some rocks. i now issue a new commandment: thou shalt do the dance. i now issue a new commandment: thou shalt do the dance. multiply your anger by about a hundred, kate, that's how much he thinks he loves you. i don't think they tried to market it to the billionaire, spelunking, base-jumping crowd. that tall drink of water with the silver spoon up his ass. it only took me six days. same time it took the lord to make the world.</p>"
		...
		directives: {
			...
			// Hierbij pakt hij alleen de <p data-bind="review"> in index.html (er is geen 'review' in movieApp.movies)
			// Hierin gaan we andere gegevens uit movieApp.movies injecteren
			review: {
				text: function() {
					// Via underscore pakken we van deze film de movieApp.movies.reviews alle parameters score
					// Underscore.js maakt een array aan van alle gevonden reviews
					var map = _.pluck(this.reviews, "score").map(function (value){ return value }); 
					// Via underscore pakt reduce alle waardes uit de variabele map, en voegt dit bij elkaar toe
					// - memo staat voor memory (dat begint met 0) <functie vanuit underscore.js>
					// - num staat voor het getal dat mee wordt gegeven uit map <functie vanuit underscore.js>
					// Achteraf delen we het getal door het aantal waardes die in de array map stonden, waardoor een gemiddelde ontstaat.
					var reduce = _.reduce(map, function(memo, num){ return memo + num; }, 0) / map.length;
					// Een shorthand if/else dat simpelweg zegt: als de variable reduce geen getal is (NaN), geef "Geen reviews" door, anders geef het getal van variable reduce door.
					// Dit is gedaan omdat de film Fight Club geen reviews heeft.
					return isNaN(reduce) ? "Geen reviews" : reduce;
				}
			},
			// Hierbij combineert hij de <div data-bind="genres"> in index.html en genres in movieApp.movies.genres
			genres: {
				// Hierbij pakt hij alleen de <p data-bind="review"> in index.html (er is geen 'genre' in movieApp.movies)
				// Hierin gaan we de waardes uit movieApp.movies.genres injecteren, omdat de waardes van movieApp.movies.genres een naamloze array is
				genre: {
					text: function() {
						// Omdat de waardes uit movieApp.movies.genres een naamloze array is, hoeven we alleen return this.value te doen
						// Dit omdat de directive een laag dieper gaat dan movieApp.movies.genres en er in movieApp.movies.genres verder geen properties zijn
						return this.value;
					}
				}
			},
